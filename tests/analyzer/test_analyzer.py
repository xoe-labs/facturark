...
def test_analyzer_get_document_type(analyzer):
    assert (
        analyzer.get_document_type(Element(QName(NS.fe, "Invoice"), nsmap=vars(NS)))
        == "1"
    )
    assert (
        analyzer.get_document_type(Element(QName(NS.fe, "CreditNote"), nsmap=vars(NS)))
        == "2"
    )
    assert (
        analyzer.get_document_type(Element(QName(NS.fe, "DebitNote"), nsmap=vars(NS)))
        == "3"
    )
    assert (
        analyzer.get_document_type(
            Element(QName(NS.fe, "ApplicationResponse"), nsmap=vars(NS))
        )
        == "4"
    )
