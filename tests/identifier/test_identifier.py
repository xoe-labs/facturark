...
def test_invoice_identifier_identify(invoice, invoice_identifier):
    ...
    assert expected_dict["technical_key"] == (invoice_identifier.technical_key)
...
def test_invoice_uuid_generator_parse_invoice(invoice, invoice_identifier):
...
def test_invoice_identifier_get_tax_values(invoice, invoice_identifier):
    tax_code_1, tax_value_1 = invoice_identifier._get_tax_values(invoice, "01")

    assert tax_code_1 == "01"
    assert tax_value_1 == "380.00"

    tax_code_2, tax_value_2 = invoice_identifier._get_tax_values(invoice, "02")

    assert tax_code_2 == "02"
    assert tax_value_2 == "0.00"

    tax_code_3, tax_value_3 = invoice_identifier._get_tax_values(invoice, "03")
...
def test_invoice_identifier_inject_uuid_hash(invoice, invoice_identifier):
    uuid_hash = "77c35e565a8d8f9178f2c0cb422b067091c1d760"

    injected_invoice = invoice_identifier._inject_uuid_hash(invoice, uuid_hash)
