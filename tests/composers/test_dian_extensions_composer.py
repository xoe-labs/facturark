...
def data_dict():
    return {
        ...
        "invoice_source": {"identification_code": "CO"},
        "software_provider": {
...
def test_compose(composer, data_dict, schema):
    ...
    assert (
        invoice_control.findtext(QName(NS.sts, "InvoiceAuthorization"))
        == "9000000500017960"
    )

    authorization_period = invoice_control.find(QName(NS.sts, "AuthorizationPeriod"))
    assert authorization_period is not None

    authorized_invoices = invoice_control.find(QName(NS.sts, "AuthorizedInvoices"))
    assert authorized_invoices is not None

    schema.assertValid(dian_extensions)


def test_compose_without_some_data(composer, data_dict_without_some_data, schema):
