...
from facturark.signer.resolver import resolve_signed_properties_composer
...
@fixture
def composer():
    return resolve_signed_properties_composer()
...
def data_dict():
    return {
        ...
        "signed_signature_properties": {
            ...
            "signer_role": {"claimed_roles": [{"claimed_role": "supplier"}]},
        },
    }
...
def test_compose(composer, data_dict, schema):
    ...
    assert signed_properties.attrib.get("Id") == (data_dict["@attributes"]["Id"])

    assert (
        signed_properties.find(QName(NS.xades, "SignedSignatureProperties")) is not None
    )
