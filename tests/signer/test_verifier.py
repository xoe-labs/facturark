...
from facturark.signer import Canonicalizer, Encoder, Encrypter, Hasher, Verifier
...
def test_verifier_get_canonicalization_method(verifier, signed_invoice):
    canonicalization_method = verifier._get_canonicalization_method(signed_invoice)
...
def test_verifier_parse_references(verifier, signed_invoice):
    ...
    reference_elements = signed_invoice.findall(".//ds:Reference", namespaces=vars(NS))
...
def test_verifier_digest_sha512_signed_properties(verifier, signed_invoice_sha512):
    uri = "#xmldsig-a05d47f2-d7a4-4964-8518-93b3bd2817d4-signedprops"
    method = "http://www.w3.org/2001/04/xmlenc#sha512"
    resource_digest = verifier._digest_resource(signed_invoice_sha512, uri, method)
...
def test_verifier_digest_sha512_keyinfo(verifier, signed_invoice_sha512):
...
def test_verifier_digest_sha512_document(verifier, signed_invoice_sha512):
    uri = ""
    method = "http://www.w3.org/2001/04/xmlenc#sha512"
    resource_digest = verifier._digest_resource(signed_invoice_sha512, uri, method)
...
def test_verifier_digest_sha512_signed_info(verifier, signed_invoice_sha512):
    ...
    signed_info_digest = verifier._digest_signed_info(signed_invoice_sha512, method)

    assert signed_info_digest is not None


def test_verifier_extract_certificate(verifier, signed_invoice_sha512, certificate_pem):

    extracted_certificate = verifier._extract_certificate(signed_invoice_sha512)

    assert extracted_certificate == certificate_pem


def test_verifier_extract_signature_value(
    verifier, signed_invoice_sha512, signature_value
):

    extracted_signature = verifier._extract_signature_value(signed_invoice_sha512)
...
def test_verifier_extract_signature_method(verifier, signed_invoice_sha512):
    ...
    extracted_signature = verifier._extract_signature_method(signed_invoice_sha512)
...
def test_verifier_compare_mismatched_digests(verifier):
    digest_a = (
        "zeO2I35ESFbtHIm1y3vG25gm1wa80VTP+JZzHt0HrW1bq1kNpdcD0KY+"
        "pQVnShAOU/QyN6tNZiAJXm4K3RhWhg=="
    )
    digest_b = (
        "4ua9R6NcHUyDwrUMv9/URwx5aomERb8SgEimLkWxb9dQByijPJslLJhm"
