...
def test_signer_parse_certificate(signer, certificate):
    ...
    assert isinstance(result, cryptography.hazmat.backends.openssl.x509._Certificate)
...
def test_signer_parse_private_key(signer, private_key):
    ...
    assert isinstance(
        result, cryptography.hazmat.primitives.asymmetric.rsa.RSAPrivateKey
    )
...
def test_signer_serialize_certificate(signer, certificate):
    ...
    assert b"".join(b64encode(b64decode(result)).split()) == (b"".join(result.split()))
...
def test_signer_prepare_key_info(signer, certificate):
    ...
    key_info, key_info_digest = signer._prepare_key_info(
        certificate_object, uid, algorithm
    )
...
def test_signer_prepare_signed_properties(signer, certificate):
    ...
    signed_properties, signed_properties_digest = signer._prepare_signed_properties(
        certificate_object, uid
    )
...
def test_signer_prepare_document(signer, unsigned_invoice):
    algorithm = "http://www.w3.org/2001/04/xmlenc#sha512"
    document_element, document_digest = signer._prepare_document(
        unsigned_invoice, algorithm
    )
...
def test_signer_prepare_signed_info(signer, unsigned_invoice):
    ...
    document_reference = signed_info.find('.//ds:Reference[@Id="ABC123"]', vars(NS))
    assert document_reference.attrib["Id"] == "ABC123"
    assert document_reference.attrib["URI"] == ""

    key_info_reference = signed_info.find('.//ds:Reference[@URI="#keyinfo"]', vars(NS))
...
def test_signer_prepare_signature_value(signer):
    ...
    signature_value = signer._prepare_signature_value(value, uid)
...
def test_create_signature_value_digest(signer, certificate, private_key):
    ...
    signer.signature_algorithm = "http://www.w3.org/2001/04/xmldsig-more#rsa-sha512"
