...
class ReferenceComposer(Composer):
    def compose(self, data_dict, root_name=None):
        ...
        root = Element(
            QName(NS.ds, root_name), data_dict.get("@attributes"), nsmap=vars(NS)
        )

        transforms_list = data_dict.get("transforms")
        if transforms_list:
            transforms = make_child(root, QName(NS.ds, "Transforms"), empty=True)
            for transform_dict in transforms_list:
                make_child(
                    transforms,
                    QName(NS.ds, "Transform"),
        ...
        make_child(root, QName(NS.ds, "DigestValue"), data_dict["digest_value"])
