...
class SignedSignaturePropertiesComposer(Composer):
    def __init__(
        ...
        self.signature_policy_identifier_composer = signature_policy_identifier_composer
    ...
    def compose(self, data_dict, root_name=None):
        ...
        make_child(root, QName(NS.xades, "SigningTime"), data_dict["signing_time"])

        signing_certificate_dict = data_dict["signing_certificate"]
        root.append(self.signing_certificate_composer.compose(signing_certificate_dict))

        signature_policy_identifier_dict = data_dict["signature_policy_identifier"]
        root.append(
            self.signature_policy_identifier_composer.compose(
                signature_policy_identifier_dict
            )
        )

        signer_role_dict = data_dict["signer_role"]
        root.append(self.signer_role_composer.compose(signer_role_dict))
