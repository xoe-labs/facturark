...
class SignatureComposer(Composer):
    ...
    def compose(self, data_dict, root_name=None):
        ...
        root = Element(
            QName(NS.ds, root_name), attrib=data_dict.get("@attributes"), nsmap=vars(NS)
        )

        if data_dict.get("signed_info"):
            root.append(self.signed_info_composer.compose(data_dict["signed_info"]))

        if data_dict.get("signature_value"):
            root.append(
                self.signature_value_composer.compose(data_dict["signature_value"])
            )
