...
def cli_build_document(options_dict):
    ...
    certificate = (
        read_file(options_dict.get("certificate"))
        if options_dict.get("certificate")
        else None
    )
    ...
    invoice_xml, _ = build_document(
        invoice_dict, certificate, private_key, technical_key, kind
    )
...
def cli_send_document(options_dict):
    ...
    response_json = json.dumps(response_dict, default=json_serialize).encode("utf-8")
    output_file = options_dict.get("output_file")
...
def parse(arg_list):
    ...
    build_parser.add_argument("-p", "--private_key")
