...
class TaxSubtotalComposer(Composer):
    ...
    def compose(self, data_dict, root_name=None):
        ...
        root.append(
            self.amount_composer.compose(data_dict["taxable_amount"], "TaxableAmount")
        )

        root.append(self.amount_composer.compose(data_dict["tax_amount"], "TaxAmount"))

        make_child(root, QName(NS.cbc, "Percent"), data_dict["percent"], required=True)

        tax_category_dict = data_dict["tax_category"]
        tax_scheme_dict = tax_category_dict["tax_scheme"]
        tax_category = make_child(
            root, QName(NS.cac, "TaxCategory"), required=True, empty=True
        )
        tax_scheme = make_child(
            tax_category, QName(NS.cac, "TaxScheme"), required=True, empty=True
        )
        make_child(
            tax_scheme, QName(NS.cbc, "ID"), tax_scheme_dict["id"], required=True
        )
