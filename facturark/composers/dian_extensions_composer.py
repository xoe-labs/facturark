...
class DianExtensionsComposer(Composer):
    def compose(self, data_dict, root_name=None):
        ...
        if data_dict.get("invoice_control"):
            ...
            make_child(
                invoice_control,
                QName(NS.sts, "InvoiceAuthorization"),
                data_dict["invoice_control"]["invoice_authorization"],
            )
            authorization_period_dict = data_dict["invoice_control"][
            ...
            make_child(
                authorization_period,
                QName(NS.cbc, "StartDate"),
                authorization_period_dict["start_date"],
            )
            make_child(
                authorization_period,
                QName(NS.cbc, "EndDate"),
                authorization_period_dict["end_date"],
            )

            authorized_invoices_dict = data_dict["invoice_control"][
                "authorized_invoices"
            ]
            ...
            make_child(
                authorized_invoices, QName(NS.sts, "To"), authorized_invoices_dict["to"]
            )

        invoice_source = make_child(root, QName(NS.sts, "InvoiceSource"), empty=True)
        make_child(
            invoice_source,
            QName(NS.cbc, "IdentificationCode"),
        ...
        make_child(
            software_provider,
            QName(NS.sts, "ProviderID"),
            data_dict["software_provider"]["provider_id"],
        )
        make_child(
            software_provider,
            QName(NS.sts, "SoftwareID"),
            data_dict["software_provider"]["software_id"],
        )

        make_child(
            root,
            QName(NS.sts, "SoftwareSecurityCode"),
