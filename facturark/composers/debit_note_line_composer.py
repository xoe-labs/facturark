...
class DebitNoteLineComposer(Composer):
    ...
    def compose(self, data_dict, root_name=None):
        ...
        make_child(discrepancy_response, QName(NS.cbc, "ReferenceID"), empty=True)
        make_child(
        ...
        for billing_reference_dict in data_dict.get("billing_references", []):
            root.append(self.billing_reference_composer.compose(billing_reference_dict))

        tax_total_dict = data_dict.get("tax_total")
        if tax_total_dict:
            tax_total = make_child(root, QName(NS.cac, "TaxTotal"), empty=True)
            tax_total.append(
                self.amount_composer.compose(tax_total_dict["tax_amount"], "TaxAmount")
            )

        item_dict = data_dict.get("item")
        if item_dict:
            item = make_child(root, QName(NS.cac, "Item"), empty=True)
            make_child(item, QName(NS.cbc, "Description"), item_dict["description"])
            make_child(
                item,
                QName(NS.cbc, "AdditionalInformation"),
