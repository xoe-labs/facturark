...
class AddressComposer(Composer):
    def compose(self, data_dict, root_name="Address"):
        ...
        make_child(
            root, QName(NS.cbc, "Department"), data_dict["department"], required=False
        )
        make_child(
            root, QName(NS.cbc, "CityName"), data_dict["city_name"], required=False
        )

        if data_dict.get("address_line"):
            address_line = make_child(
                root, QName(NS.cac, "AddressLine"), required=False, empty=True
            )
            make_child(
                address_line,
                QName(NS.cbc, "Line"),
