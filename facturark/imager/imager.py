...
class Imager:
    ...
    def qrcode(self, document):
        ...
        invoice_date = (
            self.analyzer.get_issue_date(document)
            .replace("-", "")
            .replace(":", "")
            .replace("T", "")
        )
        ...
        message_dict["invoice_amount"] = self.analyzer.get_total_line_extension_amount(
            document
        )
        message_dict["tax_vat_amount"] = self.analyzer.get_tax_vat(document)
        tax_other = "{:.2f}".format(
            sum(float(value) for value in self.analyzer.get_tax_other(document))
        )
        message_dict["tax_other_amount"] = tax_other
        message_dict["total_amount"] = self.analyzer.get_total_payable_amount(document)
