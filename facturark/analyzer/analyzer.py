...
class Analyzer:
    ...
    def get_supplier_vat(self, document):
        supplier_vat = document.find(
            ("fe:AccountingSupplierParty/fe:Party/" "cac:PartyIdentification/cbc:ID"),
            vars(NS),
        ).text
        return supplier_vat
    ...
    def get_document_number(self, document, without_prefix=False):
        ...
        if without_prefix:
            invoice_number = "".join(char for char in invoice_number if char.isdigit())
    ...
    def get_signing_time(self, document):
        signing_time = document.find(
            ".//xades:SignedSignatureProperties/xades:SigningTime", vars(NS)
        ).text
        return signing_time

    def get_software_identifier(self, document):
        software_identifier = document.find(
            ".//sts:DianExtensions/sts:SoftwareProvider/sts:SoftwareID", vars(NS)
        ).text
        return software_identifier

    def get_uuid(self, document):
        uuid = document.find(".//cbc:UUID", vars(NS)).text
        return uuid

    def get_supplier_type(self, document):
        return document.find(
            (".//fe:AccountingSupplierParty/" "cbc:AdditionalAccountID"), vars(NS)
        ).text

    def get_customer_type(self, document):
        return document.find(
            (".//fe:AccountingCustomerParty/" "cbc:AdditionalAccountID"), vars(NS)
        ).text
    ...
    def get_supplier_country(self, document):
        return document.find(
            (
                ...
                "fe:Address/cac:Country/cbc:IdentificationCode"
            ),
            vars(NS),
        ).text
    ...
    def get_document_currency(self, document):
        return document.find((".//cbc:DocumentCurrencyCode"), vars(NS)).text
    ...
    def get_tax_total_amounts(self, document):
        result = []
        for tax_element in document.findall((".//fe:TaxTotal/cbc:TaxAmount"), vars(NS)):
    ...
    def get_tax_types(self, document):
        ...
        for tax_element in document.findall(
            (".//fe:TaxSubtotal/cac:TaxCategory/" "cac:TaxScheme/cbc:ID"), vars(NS)
        ):
            result.append(tax_element.text)
        return result
    ...
    def get_customer_id(self, document):
        id = document.find(
            ".//fe:AccountingCustomerParty/fe:Party/" "cac:PartyIdentification/cbc:ID",
            vars(NS),
        ).text
        return id
    ...
    def get_total_line_extension_amount(self, document):
        return document.find(
            (".//fe:LegalMonetaryTotal/" "cbc:LineExtensionAmount"), vars(NS)
        ).text

    def get_total_tax_exclusive_amount(self, document):
        return document.find(
            (".//fe:LegalMonetaryTotal/" "cbc:TaxExclusiveAmount"), vars(NS)
        ).text

    def get_total_payable_amount(self, document):
        return document.find(
            (".//fe:LegalMonetaryTotal/" "cbc:PayableAmount"), vars(NS)
        ).text
    ...
    def get_software_provider_id(self, document):
        return document.find(
            (".//sts:DianExtensions/" "sts:SoftwareProvider/sts:ProviderID"), vars(NS)
        ).text
